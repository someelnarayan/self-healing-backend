...
Important:
- Cooldown is handled by main.py after execution,
  NOT inside executor.py
"""
...
def restart_process(
    ...
    print(
        f"[Executor] SSH restart_process on {target.name}: "
...
def cleanup_demo(
    ...
    print(
        f"[Executor] SSH cleanup_demo on {target.name}: "
        f"{target.cleanup_command}",
        flush=True,
    )
...
class Executor:
    ...
    def execute(
        ...
        # Cooldown is handled in main.py, not here
        # --------------------------------------------------------
        if overall_success:
            self.kb.update_incident_status(
                anomaly.target_name,
